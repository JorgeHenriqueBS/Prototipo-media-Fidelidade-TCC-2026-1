...
interface FoodItem {
  ...
  name: string;
  value: number;
  icon: 'tucuma' | 'buriti' | 'cupuacu' | 'jambo' | 'acai';
  placed: boolean;
}
...
interface TreeNodeData {
  ...
  icon: 'tucuma' | 'buriti' | 'cupuacu' | 'jambo' | 'acai' | null;
}

interface BSTGameLevel2Props {
  onNextLevel: () => void;
}
...
export function BSTGameLevel2({ onNextLevel }: BSTGameLevel2Props) {
  ...
  const [foods, setFoods] = useState<FoodItem[]>([
    ...
    { id: 'acai', name: 'Açaí', value: 5, icon: 'acai', placed: false },
  ]);
  ...
  const [tree, setTree] = useState<TreeNodeData[]>([
    ...
    { id: 'left-l3', value: null, foodName: null, icon: null },
    { id: 'right-l3', value: null, foodName: null, icon: null },
  ]);
  ...
  const checkSolution = () => {
    // Solução correta: root=3, left-l2=2, right-l2=4, left-l3=1, right-l3=5
    const correct = 
      tree[0].value === 3 &&
      tree[1].value === 2 &&
      tree[2].value === 4 &&
      ...
      >
        ...
        <div className="relative flex-1 flex items-start justify-center pt-8 pb-2">
          ...
          <div className="relative">
            ...
            <div className="absolute top-[24px] left-1/2 -translate-x-1/2 w-[320px] h-[90px] pointer-events-none z-10">
              <svg className="absolute left-0 top-0 w-full h-full" style={{ imageRendering: 'pixelated' }}>
                ...
                <path
                  d="M 125 22 L 90 22 Q 70 22, 70 42 L 70 90"
                ...
                <path
                  d="M 195 22 L 230 22 Q 250 22, 250 42 L 250 90"
                  stroke="url(#weavePatternL2)"
                  strokeWidth="16"
                  fill="none"
                  strokeLinecap="round"
            ...
            <div className="absolute top-[98px] left-1/2 -translate-x-1/2 w-[400px] h-[90px] pointer-events-none z-10">
              <svg className="absolute left-0 top-0 w-full h-full" style={{ imageRendering: 'pixelated' }}>
                <defs>
                ...
                {/* Conexão esquerda: nó esquerdo nível 2 -> nó esquerdo nível 3 */}
                <path
                  d="M 53 22 L 28 22 Q 8 22, 8 42 L 8 90"
                ...
                {/* Conexão direita: nó direito nível 2 -> nó direito nível 3 */}
                <path
                  d="M 347 22 L 372 22 Q 392 22, 392 42 L 392 90"
                  stroke="url(#weavePatternL3)"
                  strokeWidth="16"
                  fill="none"
                  strokeLinecap="round"
            ...
            <div className="flex justify-center gap-[300px]">
              <TreeNode
                node={tree[3]}
                onDrop={(food) => handleDrop('left-l3', food)}
                onRemove={() => handleRemove('left-l3')}
                size={88}
              />
              <TreeNode
                node={tree[4]}
                onDrop={(food) => handleDrop('right-l3', food)}
                onRemove={() => handleRemove('right-l3')}
                size={88}
              />
            </div>
          </div>
